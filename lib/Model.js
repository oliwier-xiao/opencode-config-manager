...
function setRoster(roster) {
  if (!isObject(roster)) return
  var oc = isObject(roster.opencode) ? roster.opencode : {}
  var om = isObject(roster.ohmy) ? roster.ohmy : {}
  if (Array.isArray(oc.agents) && oc.agents.length) OPENCODE_AGENTS = oc.agents.slice()
  if (Array.isArray(om.agents) && om.agents.length) OHMY_AGENTS = om.agents.slice()
  if (Array.isArray(om.categories) && om.categories.length) OHMY_CATEGORIES = om.categories.slice()
}
...
function setShape(shape) {
  if (typeof shape === "string" && shape) SHAPE = shape
}
