...
// reads the real rosters off the installed software — oh-my-openagent's own
// `dist/oh-my-opencode.schema.json` and opencode's own `opencode generate`
...
var OPENCODE_AGENTS = ["build", "plan", "general", "explore"]
...
// The spelling this entry already uses. A new entry gets `variant`, which every 3.x and
// 4.x understands; `reasoning` is written back only where the entry already chose it.
function effortKey(value) {
  return (isObject(value) && !Array.isArray(value) && value.reasoning !== undefined)
    ? "reasoning" : "variant"
}
...
// so an `agent` entry is always written long, even for a lone model. The short
// string form stays available everywhere it is legal: opencode's own top-level
// `model`/`small_model`, and every oh-my-openagent agent and category.
...
// promoting every entry on the first edit would rewrite half the file to
// change one model.
function setRowModel(profile, row, modelId) {
  ...
  else if (longFormOnly(row)) c[row.key] = orderedAssign(typeof value === "string" && value ? { model: value } : {}, { model: modelId })
...
function setRowVariant(profile, row, variant) {
  ...
  // unless the file demands the long form, where collapsing it would be invalid.
  if (!longFormOnly(row) && Object.keys(value).length === 1 && value.model !== undefined) c[row.key] = value.model
