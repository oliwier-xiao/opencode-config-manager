...
var OHMY_AGENTS = [
  "sisyphus", "sisyphus-junior", "prometheus", "metis", "momus", "oracle",
  "explore", "librarian", "atlas", "hephaestus", "multimodal-looker"
]
var OHMY_CATEGORIES = [
  "visual-engineering", "artistry", "ultrabrain", "deep",
  "quick", "unspecified-low", "unspecified-high", "writing"
]
var OPENCODE_AGENTS = ["build", "plan", "general", "explore", "scout"]
...
function rowsFor(profile) {
  ...
  if (oc) {
    rows.push(makeRow("opencode", "default", "Default model", oc, "model"))
    rows.push(makeRow("opencode", "default", "Small model", oc, "small_model"))
    keys = rosterKeys(oc.agent, OPENCODE_AGENTS)
    for (i = 0; i < keys.length; i++) {
      rows.push(makeRow("opencode", "agent", cleanName(keys[i]), isObject(oc.agent) ? oc.agent : {}, keys[i]))
    }
  }
  if (om) {
    keys = rosterKeys(om.agents, OHMY_AGENTS)
    for (i = 0; i < keys.length; i++) {
      rows.push(makeRow("ohmy", "agent", cleanName(keys[i]), isObject(om.agents) ? om.agents : {}, keys[i]))
    }
    keys = rosterKeys(om.categories, OHMY_CATEGORIES)
    for (i = 0; i < keys.length; i++) {
      rows.push(makeRow("ohmy", "category", cleanName(keys[i]), isObject(om.categories) ? om.categories : {}, keys[i]))
    }
  }
  return rows
}
...
// opencode validates `agent.<name>` as an object and refuses the WHOLE file over a
// bare string there — providers, MCP servers and keys go down with it. `model` and
// `small_model` are the opposite: they want the bare string. oh-my-openagent takes
// either, so only opencode's agent rows are forced.
function objectOnly(row) { return row.file === "opencode" && row.group === "agent" }
...
function setRowModel(profile, row, modelId) {
  ...
  else c[row.key] = objectOnly(row) ? { model: modelId } : modelId
  return p
}
...
function setRowVariant(profile, row, variant) {
  ...
  // An object whose only key is `model` is a string with extra punctuation — except
  // where opencode demands the object, and collapsing there is what took the file down.
  if (!objectOnly(row) && Object.keys(value).length === 1 && value.model !== undefined) c[row.key] = value.model
