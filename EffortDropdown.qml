...
import qs.Commons
// Not in the shell's own copy: there this file sits inside qs.Ui and resolves
// BorderSurface and the rest from its own module. Out here it has to ask.
import qs.Ui

// A fork of the shell's Ui/Dropdown, verbatim but for its closePolicy — the
// same reason ModelPicker is a fork of Ui/SearchableDropdown. The shell's
// version leaves closePolicy at the default, and because its popup is a child
// of its own trigger, CloseOnPressOutside shuts the list on the *press* and
// leaves the release to reopen it: the effort control could be opened by
// clicking and never closed by clicking. Only the one line below differs, so
// re-syncing against the shell's file stays a one-line reapply.
//
// ---- the shell's own notes follow ----
//
  ...
  Column {
    ...
    BorderSurface {
      ...
      Popup {
        ...
        focus: true
        // The one line this fork exists for. See the header.
